(function () {
    ...
    function UserService($http, $q, $location, FlashService) {
        ...
        function GetUserSearch(credentials, user) {
            ...
            var promise = $http.get('http://localhost:60792/api/usersearch/' + user.username, {cache: false}).then(function(response){
            
            if(response.data.Result){
                ...
                $http.post('http://localhost:60792/api/users/', JSONObject).then(handleSuccess("Success"), handleError('Error creating user'));
                ReadyToLogin = true;
                $location.path('/main');
                debugger;
...
            });
        ...
        function GetByUsername(username) {
            debugger;
            var promise = $http.get('http://localhost:60792/api/usersbyname/' + username, {cache: false}).then(function(response){             
...
            });
        ...
        function GetByCredentials(credentials) {
            var promise = $http.get('http://localhost:60792/api/credentials/' + credentials, {cache: false}).then(function(response){
                return response.data;
                });
            return promise;
        }  
